...
class Planetoids(GameApp):
    ...
    #            only None if _state is STATE_ACTIVE.
     # Attribute _background: game background
     # Attribute _stars: game background artifacts (stars)
    # END REMOVE
    ...
    def start(self):
        ...
        """
        self._title = GLabel(x=self.width//2, y=(self.height//2)+TITLE_OFFSET, 
        text="PLANETOIDS", font_name=TITLE_FONT, font_size=TITLE_SIZE)
        self._message = GLabel(x=self.width//2,
        y=(self.height//2)+MESSAGE_OFFSET, 
        text="PRESS 'S' KEY TO START", font_name=MESSAGE_FONT,
        font_size=MESSAGE_SIZE)
        self._state = STATE_INACTIVE 
        self._sdown = False
        self._wave = None
        self._lastkeys = 0
        self._background = GRectangle(
            x=GAME_WIDTH//2, y=GAME_HEIGHT//2,
            fillcolor="dark blue",height=GAME_HEIGHT,width=GAME_WIDTH
        )
    ...
    def update(self,dt):
        ...
        if self._state is STATE_INACTIVE:
            print("INACTIVE")
        if self._state is STATE_LOADING:
            self._message = None
            self._title = None
            print("LOADING")
            wavej = self.load_json(DEFAULT_WAVE)
            self._wave = Wave(wavej,3)
        if self._state is STATE_ACTIVE:
            if not self._wave is None:
                self._wave.update(dt,self.input)
        if self._state is STATE_PAUSED:
            print("ACTIVE")
            self.pauseMessage()
        if self._state is STATE_CONTINUE:
            print("CONTINUE")
            self._wave.setShip(self.load_json(DEFAULT_WAVE))
            self._message = None
        if self._state is STATE_COMPLETE and len(self._wave.getAsteroids())==0:
            self.endMessage()
        if self._state is STATE_COMPLETE and self._wave.getLives()==0:
            self.endMessage()
    ...
    def endMessage(self):
        """
        end message for win or loss, depending on the lives or number of
        asteroids left
        """
        if len(self._wave.getAsteroids()) == 0:
            self._message = GLabel(x=self.width//2,
                y=(self.height//2)+ MESSAGE_OFFSET, 
                text="YOU WON!", font_name=MESSAGE_FONT,
                font_size=MESSAGE_SIZE)
        else:
            self._message = GLabel(x=self.width//2,
            y=(self.height//2)+ MESSAGE_OFFSET, 
            text="YOU LOST!", font_name=MESSAGE_FONT,
            font_size=MESSAGE_SIZE)


    def pauseMessage(self):
        """Message for paused state.
        """
        self._message = GLabel(x=self.width//2,
        y=(self.height//2)+ MESSAGE_OFFSET, 
            text="PRESS 'S' TO CONTINUE", font_name=MESSAGE_FONT,
            font_size=MESSAGE_SIZE)

    def _determineState(self):
        """Determines the current state of the game based on various conditions.
    This function checks the current state of the game and the state of various
    objects, such as the wave and ship, to determine the current state of the game.
    The state can be one of the following: STATE_INACTIVE, STATE_LOADING,
    STATE_ACTIVE, STATE_PAUSED, STATE_CONTINUE, or STATE_COMPLETE. The function
    updates the value of the `_state` attribute based on the current conditions.
    """
        if not self._wave is None:
            oglives = self._wave.getLives()
        if self._sdown:
            if self._state is STATE_INACTIVE:
                self._state = STATE_LOADING
        if self._state is STATE_LOADING and not self._wave is None and \
         not self._wave.getShip() is None:
            self._state = STATE_ACTIVE
        if self._state is STATE_CONTINUE and not self._wave.getShip() is None:
            self._state = STATE_ACTIVE
        ...
        if not self._wave is None:
            ...
            if len(self._wave.getAsteroids()) == 0:
                self._state = STATE_COMPLETE
        if self._state is STATE_PAUSED and self._sdown:
